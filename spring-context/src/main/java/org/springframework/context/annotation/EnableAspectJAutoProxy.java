 ...
 * to functionality found in Spring's {@code <aop:aspectj-autoproxy>} XML element. To be used on
 ...
 * }</pre>
 * <p>
 * Where {@code FooService} is a typical POJO component and {@code MyAspect} is an {@code
 * @Aspect}-style aspect:
 *
 * <pre class="code">
 ...
 * the {@link #proxyTargetClass()} attribute. The following enables CGLIB-style 'subclass' proxies
 * as opposed to the default interface-based JDK proxy approach.
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 * allowing for selective proxying of beans at different levels.</b> Please redeclare {@code
 * @EnableAspectJAutoProxy} in each individual context, e.g. the common root web application context
 * and any separate {@code DispatcherServlet} application contexts, if you need to apply its
 * behavior at multiple levels.
 *
 * <p>This feature requires the presence of {@code aspectjweaver} on the classpath.
 * While that dependency is optional for {@code spring-aop} in general, it is required for {@code
 * @EnableAspectJAutoProxy} and its underlying facilities.
 *
 * @author Chris Beams
